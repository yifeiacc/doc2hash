...
model = VDSH_gubel(dataset, num_features, num_bits, dropoutProb=args.dropout, device=device)
model.to(device)
...
with open('logs/VDSH/loss.log.txt', 'w') as log_handle:
    ...
    for epoch in range(num_epochs):
        ...
        for step, (xb, yb) in enumerate(train_loader):
            ...
            kl_loss = VDSH_gubel.calculate_KL_loss(qy)
            reconstr_loss = VDSH_gubel.compute_reconstr_loss(logprob_w, xb)
        ...
        with torch.no_grad():
            ...
            if prec.item() > best_precision:
                ...
                with open("plot_z_gumbel_best.pk", "wb") as f:
                    pickle.dump(plot_z, f)

            if epoch == num_epochs - 1:
                with open("plot_z_gumbel_last.pk","wb") as f:
                    pickle.dump(plot_z, f)
